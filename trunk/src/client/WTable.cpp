	...
	if (seat->valid)
	{
		...
		if (seat->in_round)
		{
			...
			if (allcards.size())
			{
				...
				// GABE Makes hole cards completely dissapear
			...
			// GABE makes the little backs of the cards dissapear when false
			if (snap->my_seat == (int)s)
...
{
	// GABE this is displaying the hole cards...
	if (!protected_mode)
...
QString WTable::buildHandStrengthString(HandStrength *strength, int verbosity)
{
	...
	//changed by GABE so that the client doesnt display the hand strength
	//return retstr;
	return "";
}
