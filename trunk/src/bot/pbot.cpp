...
void PBot::serverCmdPserver(Tokenizer &t)
{
...
void PBot::serverCmdErr(Tokenizer &t)
{
...
void PBot::serverCmdMsg(Tokenizer &t)
{
...
// table snapshot
void PBot::serverCmdSnapTable(Tokenizer &t, int gid, int tid, tableinfo* tinfo)
{
	// silently drop message if there is no table-info
	if (!tinfo)
		return;
...
void PBot::serverCmdSnapGamestate(Tokenizer &t, int gid, int tid, tableinfo* tinfo)
{
...
// cards snapshot
void PBot::serverCmdSnapCards(Tokenizer &t, int gid, int tid, tableinfo* tinfo)
{
...
void PBot::serverCmdSnapPlayerAction(Tokenizer &t, int gid, int tid, tableinfo* tinfo)
{
	// silently drop message if there is no table-info
	if (!tinfo || !tinfo->window)
		return;
...
void PBot::serverCmdSnapPlayerShow(Tokenizer &t, int gid, int tid, tableinfo* tinfo)
{
	// silently drop message if there is no table-info
	if (!tinfo || !tinfo->window)
		return;
...
void PBot::serverCmdSnapFoyer(Tokenizer &t)
{
...
// server cmd SNAP
void PBot::serverCmdSnap(Tokenizer &t)
{
...
void PBot::serverCmdPlayerlist(Tokenizer &t)
{
	char msg[1024];
...
void PBot::serverCmdClientinfo(Tokenizer &t)
{
	const int cid = t.getNextInt();
...
void PBot::serverCmdGameinfo(Tokenizer &t)
{
	const int gid = t.getNextInt();
...
void PBot::serverCmdGamelist(Tokenizer &t)
{
	// game-list
	std::string sreq;
...
int PBot::serverExecute(const char *cmd)
{
	Tokenizer t(" ");
...
bool PBot::addTable(int gid, int tid)
{
...
bool PBot::doConnect(QString strServer, unsigned int port)
{
...
void PBot::doRegister(int gid, bool bRegister, const QString& password)
{
	char msg[1024];
...
bool PBot::doSetAction(int gid, Player::PlayerAction action, chips_type amount)
{
	if (!connected)
		return false;
...
void PBot::chatAll(const QString& text)
{
	// foyer chat
	if (!connected)
		return;
...
void PBot::chat(const QString& text, int gid, int tid)
{
	if (!connected)
		return;
...
bool PBot::createGame(gamecreate *createinfo)
{
	char msg[1024];
...
tableinfo* PBot::getTableInfo(int gid, int tid)
{
...
playerinfo* PBot::getPlayerInfo(int cid)
{
...
QString PBot::getPlayerName(int cid)
{
...
int PBot::netSendMsg(const char *msg)
{
	char buf[1024];
...
void PBot::netError(QAbstractSocket::SocketError socketError)
{
...
void PBot::netConnected()
{
	...
	char msg[1024];
	snprintf(msg, sizeof(msg), "PBot %d %s",
		VERSION,
...
void PBot::requestGamelist()
{
	// query gamelist
	netSendMsg("REQUEST gamelist");
}

bool PBot::isGameInList(int gid)
{
...
void PBot::requestGameinfo(const char *glist)
{
	char msg[1024];
...
void PBot::requestGameinfo(int gid)
{
	char msg[1024];
...
void PBot::translate_hn_to_pp_state(Tokenizer &t, int snaptype, tableinfo* tinfo)
{
	...
	switch( snaptype ) {
		...
		case SnapTable:
			...
			table.state = st.getNextInt();
			//table.betting_round = st.getNextInt();
			msgqueue.setStatusStage(st.getNextInt());
		...
		case SnapCards:
			...
			else if (type == SnapCardsFlop || type == SnapCardsTurn || type == SnapCardsRiver)
			{
				...
				if (type == SnapCardsFlop)
					msgqueue.setStatusFlopCards(card1, card2, card3);
				else if (type == SnapCardsTurn)
					msgqueue.setStatusTurnCard(card1);
				else if (type == SnapCardsRiver)
